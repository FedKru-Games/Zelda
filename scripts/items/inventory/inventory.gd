...
func get_items() -> Array[InventoryCell]:
	return _items
...
func add_item(item_id: String, quantity: int = 0) -> bool:
	var index = 0
	for item in _items:
		if not item.is_empty() and item.content.item_id == item_id:
			item.content.quantity += quantity
	...
	for item in _items:
		if item.is_empty():
			item.content = ItemStack.create(item_id, quantity)
			inventory_cell_changed.emit(index, item)
			if index == selected_slot:
				selected_item_changed.emit(get_item_stack(index), selected_slot)
			return true
